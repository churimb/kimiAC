...
func (ac Ac) batchInsert(items []map[string][]byte) {
	for _, item := range items {
		ac.insert(item["keyword"], item["signature"])
	}
}

func (ac Ac) insert(s []byte, signature []byte)  {
	chars := []rune(string(s))
	iter := ac.root
	for _, c := range chars {
...
func (ac Ac) Match(content string) (results [][]byte) {
	chars := []rune(content)
	iter := ac.root   // 从根节点开始
	...
	for _, c := range chars {  // 逐个词开始比较
		...
		if _, ok = iter.next[c]; ok {  // 存在
			...
			if iter.isend {
				...
				results = append(results, iter.signature)
			}
		}
	}
	return
